...
public class CarMovement : MonoBehaviour
{
    ...
    /// <summary>
    /// Event for when the car hit another car
    /// </summary>
    public event System.Action HitCar;

    /// <summary>
    /// Event for when the car goes off track
    /// </summary>
    public event System.Action OffTrack;

    //Movement constants
    private const float MAX_VEL = 200.0f;
    ...
    public float NomalizedVelocity
    {
        get { return Velocity / MAX_VEL; }
        private set { }
    }
    ...
    void OnCollisionEnter2D(Collision2D other)
    {
        ...
        else if (other.transform.gameObject.layer == LayerMask.NameToLayer("Car"))
        {
            if (HitCar != null)
                HitCar();
        }
    ...
    {
        if (collision.transform.gameObject.tag == "OffTrack")
        {
            if (OffTrack != null)
                OffTrack();
        }
